...
SERVICE_NAME=home-control-monitor-hue

DATE=`date '+%Y-%m-%d %H:%M:%S'`
echo "Home control Hue monitor agent starting at ${DATE}" | systemd-cat -t ${SERVICE_NAME} -p info
...
DATE=`date '+%Y-%m-%d %H:%M:%S'`
echo "Home control Hue monitor agent terminating at ${DATE}" | systemd-cat -t ${SERVICE_NAME} -p info
