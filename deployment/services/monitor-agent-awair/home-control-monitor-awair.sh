...
echo "Home control Awair monitor agent starting at ${DATE}" | systemd-cat -t ${SERVICE_NAME} -p info
...
DATE=`date '+%Y-%m-%d %H:%M:%S'`
echo "Home control Awair monitor agent terminating at ${DATE}" | systemd-cat -t ${SERVICE_NAME} -p info
